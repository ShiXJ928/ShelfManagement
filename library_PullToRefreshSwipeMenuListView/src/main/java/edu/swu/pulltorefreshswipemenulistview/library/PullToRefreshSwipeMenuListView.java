...
public class PullToRefreshSwipeMenuListView extends ListView implements OnScrollListener {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if ((mTouchView == null || !mTouchView.isActive()) && Math.pow(dx, 2) / Math.pow(dy, 2) <= 3) {
				if (mEnablePullRefresh&&getFirstVisiblePosition() == 0 && (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
					...
					invokeOnScrolling();
				} else if (mEnablePullLoad&&(mFooterView.getBottomMargin() > 0 || deltaY < 0)) {
		...
		case MotionEvent.ACTION_UP:
			mLastY = -1; // reset
			if (lastVisibleItem == mTotalItemCount &&mEnablePullLoad && mFooterView.getHeight()>0 && mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA) {
				startLoadMore();
				resetFooterHeight();
				new ResetHeaderHeightTask().execute();
			} else if (getFirstVisiblePosition() == 0) {
				// invoke refresh
				if (mEnablePullRefresh && mHeaderView.getVisiableHeight() > mHeaderViewHeight/2) {
					mPullRefreshing = true;
					mHeaderView.setState(PullToRefreshListHeader.STATE_REFRESHING);
					if (mListViewListener != null) {
						SimpleDateFormat df = new SimpleDateFormat("MM-dd HH:mm", Locale.getDefault());
						RefreshTime.setRefreshTime(ctx.getApplicationContext(), df.format(new Date()));
						mListViewListener.onRefresh();
						setRefreshTime(RefreshTime.getRefreshTime(ctx.getApplicationContext()));
					}
				}
				resetHeaderHeight();
			}
